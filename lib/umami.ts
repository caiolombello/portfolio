...
export async function fetchUmamiStats(startAt: number, endAt: number): Promise<UmamiStats | null> {
  try {
...
export async function fetchUmamiActiveVisitors(): Promise<number> {
  try {
    ...
    const response = await fetch(`${UMAMI_CONFIG.baseUrl}/websites/${UMAMI_CONFIG.websiteId}/active`, { headers })

    if (!response.ok) {
      throw new Error(`Erro ao buscar visitantes ativos: ${response.statusText}`)
    }
...
export async function fetchUmamiPageviews(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Erro ao buscar visualizações de página: ${response.statusText}`)
    }

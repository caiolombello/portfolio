...
export async function GET(request: Request, context: { params: { username: string } }) {
  ...
  try {
    const res = await fetch(credlyUrl, { headers: { "Accept": "application/json" } });
    if (!res.ok) {
    ...
    if (!data || !data.data || !Array.isArray(data.data)) {
      return NextResponse.json({ error: "Invalid Credly data format" }, { status: 500 });
    }

    // Sanitize the data to ensure we only have the fields we need
    ...
    return NextResponse.json({ error: "Failed to fetch Credly badges" }, { status: 500 });
  }
} 